...
func NewCmdHTML(f *cmdutil.Factory) *cobra.Command {
	var sandboxID string
	var messageID string

	cmd := &cobra.Command{
		Use:   "html",
		Short: "Get HTML body of a message",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := cmdutil.AccountPath("inboxes", fmt.Sprintf("%s", sandboxID), "messages", fmt.Sprintf("%s", messageID), "body.html")
