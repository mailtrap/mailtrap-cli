...
func NewCmdTransactional(f *cmdutil.Factory) *cobra.Command {
	return newSendCmd(f, "transactional", "Send a transactional email", client.BaseTransactional)
}

func newSendCmd(f *cmdutil.Factory, name, short string, base client.BaseURL) *cobra.Command {
	var (
		from         string
		to           []string
		subject      string
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := c.Post(context.Background(), base, "/api/send", req, &resp); err != nil {
				return err
			}
